...
- Native Messaging: Chrome talks to an installed native messaging host.
...
3. Reload the unpacked extension from `dist/chrome-mv3/`.
...
4. For HTTP/SSE/bridge transports, click `授权` and approve the Chrome host permission.
...
- `mcp_native_host_unavailable`: install or fix the Chrome native messaging host manifest.
